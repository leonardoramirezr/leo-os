...
// in Chrome and Firefox, mp4 in Safari — since Groq takes both. It is only open while dictating:
...
export async function record(): Promise<Recording> {
	if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === 'undefined') {
		throw new MicrophoneError(t.cantRecord);
	}
	...
	try {
		...
		throw new MicrophoneError(
			name === 'NotAllowedError' || name === 'SecurityError' ? t.microphoneDenied : t.microphoneFailed
		);
	}
...
/**
 * Keeps the screen on while recording. A long message dictated without touching the phone would
